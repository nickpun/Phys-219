#Based  of  script by Evan Kiefl, modified  by Rob Kiefl and Evan Thomas
import numpy as np
...
# for the chi2 fit to be successful. An exponetial deacy function 3 parameters:
# amplitude, relaxation time and an   votlage offset. Initial guesses for these
# parameters are housed in guesses. For example, guesses = (3., .01. 0.01)
# corresponds to an expoential realxation  with amplitude 0.3 V at  the start 
#time,   relaxation #time of 0.01 s   and offset of 0.01 V. If the guesses are 
...
y_sigma = 0.0005
#x0 is the start time in the data.  
t0=0.00
...
def fit_function(x, amplitude, tau, V_offset):
#    """ sine function """
#    return amplitude * np.exp(-(x-t0)/tau )
   return amplitude * np.exp(-(x-t0)/tau ) + V_offset

#load the file "fname", defined above
data = np.loadtxt(fname, delimiter=',', comments='#',usecols=(0,1))
#data = np.loadtxt(fname, delimiter=',', comments='#')
# access the data columns and assign variables xraw,yraw
xraw = data[:,0]
yraw = data[:,1]
...
#param_names = ['amplitude','relaxation time']
...
# plots y, y_fitfunc, and y_guess on the same plot.
...
plt.xlabel('{} [{}]'.format(x_name,x_units))
plt.ylabel('{} [{}]'.format(y_name,y_units))
#plt.title(r'This is a title with latex $V(t)=A\exp[-(t-t_0)/\tau]+V_{offset}$')
plt.title(r'Voltage decay curve with 1M Ohm resistor')
...
if y_sigma != None:
    ...
    ax2.vlines(-y_sigma,0,np.max(hist)*1.3,lw=3,color='r')
    ax2.vlines(+y_sigma,0,np.max(hist)*1.3,lw=3,color='r',label='+/- error')
