...
x_name = 'V_diode'
x_units = 'Volts'
y_name = 'Absolute Value of Current'
y_units = 'Amps'
...
def fit_function(x, I0, kBovere):
    return  np.abs(I0*(np.exp(x/(kBovere*T))-1))
...
y = data[:,1]
y_sigma = data[:,2]
#adjust  the scaling of y_sigma  if  you want
y_sigma=1.*y_sigma
...
plt.xlabel('{} [{}]'.format(x_name,x_units))
plt.ylabel('{} [{}]'.format(y_name,y_units))
plt.title(r'Absolute Value of Current vs. V_diode')
